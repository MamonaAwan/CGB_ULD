...
class Ada_HeatMap(torch.nn.Module):
    ...
    def __init__(self, out_res):
        super(Ada_HeatMap, self).__init__()
        print('The size of heatmap is adaptive w.r.t Cosine Similarity')
        y,x = torch.meshgrid([torch.arange(0,out_res).float(), torch.arange(0,out_res).float()])
        self.x = x
        self.y = y
        self.out_res = out_res
    
    def forward(self, pts, max_csim):
        ...
        s_max, s_min = 5, 0.2   # range for sigma 0.2 to 5
        s_range = s_max - s_min
        x_min = 1/np.exp(1)
        range_factor = s_range/(1-x_min)  # x range is from 1/exp(1) to 1
        d = max_csim
        d = torch.nan_to_num(d, nan=1, posinf=1, neginf=0)
        sigma = 1/torch.exp(max_csim)
        sigma = ((sigma-x_min)*range_factor) + s_min
        sigma = (sigma).unsqueeze(2).unsqueeze(3)
        sigma = sigma.to(dtype=hms.dtype).to('cuda')
        hms = torch.exp(torch.div(hms, sigma))
        return hms
