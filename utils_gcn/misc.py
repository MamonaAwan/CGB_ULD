...
def write_meta(ofn, logger, idx2lb, inst_num=None):
    if len(idx2lb) == 0:
    ...
    logger.info('#discard: {}, #lbs: {}'.format(discard_lb, current_lb))
    logger.info('#inst: {}, #class: {}'.format(inst_num, cls_num))

    if ofn is not None:
        print('save label to', ofn)
        logger.info('save label to {}'.format(ofn))
